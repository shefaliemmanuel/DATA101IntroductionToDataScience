...
#str(mydataMixedUp)
#gp
summary(mydata[,c(6,10)])
normalize <- function(x){
  + return( (x-min(x)) / (max(x)-min(x))) }
normalize(c(1,2,3,4,5))
#mydata_Normalized <- as.data.frame(lapply(mydata[,c(1,2,3,6,7,8,10)], normalize))
...
#mydata_train_target <- mydata[1:871, 12]
#mydata_test_target <- mydata[872:891, 12]
